...
const { fetchAndSave, getLocalStatus, PATHWAY_CONFIG } = require('./lib/bio_data_fetcher.cjs');
...
// 优先从 dist/ 提供构建后的前端产物，回退到项目根目录
const DIST_DIR = path.join(ROOT_DIR, 'dist');
const STATIC_DIRS = [DIST_DIR, ROOT_DIR]; // 按优先级搜索
...
/**
 * 处理静态文件请求
 * 优先从 dist/ 查找（构建产物），回退到项目根目录（data/ 等）
 */
function handleStatic(req, res, urlPath) {
  // 根路径 → 下载页
  const targetFile = (urlPath === '/' || urlPath === '/index.html') ? '/download.html' : urlPath;

  // 安全检查：防止路径穿越
  const resolved = path.resolve(ROOT_DIR, targetFile.slice(1));
  ...
  for (const dir of STATIC_DIRS) {
    const candidate = path.join(dir, targetFile.slice(1));
  ...
  if (!foundPath) {
    // SPA 回退：未找到的路径返回 index.html（用于前端路由）
